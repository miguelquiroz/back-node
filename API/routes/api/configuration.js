...
router.get('/', function(req, res, next) {
    res.render('api', { message: "Welcome to the API", title: 'Configuration API' });
});
...
router.post('/Save', validator.body(configurationSchema), (req, res, next) => {
    dynamo.Upsert(req.body, req.body.userID, "configuration")
...
router.post('/Get',validator.body(configurationGetSchema), (req, res, next) => {
    ...
    dynamo.Get(getparams, "configuration")
    .then((result) => res.json(result))
    .catch((err) => res.json(err));
});
